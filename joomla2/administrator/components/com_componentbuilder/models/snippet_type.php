...
{    
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_componentbuilder.snippet_type';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		if ($id != 0 && (!$user->authorise('snippet_type.edit.state', 'com_componentbuilder.snippet_type.' . (int) $id))
			|| ($id == 0 && !$user->authorise('snippet_type.edit.state', 'com_componentbuilder')))
		{
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('snippet_type.delete', 'com_componentbuilder.snippet_type.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('snippet_type.edit.state', 'com_componentbuilder.snippet_type.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('snippet_type.edit', 'com_componentbuilder.snippet_type.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('snippet_type.edit',  'com_componentbuilder');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__componentbuilder_snippet_type'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_componentbuilder.edit.snippet_type.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('snippet_type.edit.state'))
		{
				$values['published'] = 0;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('snippet_type.edit.state'))
		{
