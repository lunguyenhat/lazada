...
class ComponentbuilderControllerComponent_updates extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('component_updates.edit', 'com_componentbuilder.component_updates.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('component_updates.edit.own', 'com_componentbuilder.component_updates.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('component_updates.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
		...
		return $user->authorise('component_updates.edit', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Component_updates', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=components_updates' . $this->getRedirectToListAppend(), false));
