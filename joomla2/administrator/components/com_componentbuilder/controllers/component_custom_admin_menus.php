...
class ComponentbuilderControllerComponent_custom_admin_menus extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			if (!$permission)
			{
				if ($user->authorise('component_custom_admin_menus.edit.own', 'com_componentbuilder.component_custom_admin_menus.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('component_custom_admin_menus.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
		...
		return $user->authorise('component_custom_admin_menus.edit', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Component_custom_admin_menus', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=components_custom_admin_menus' . $this->getRedirectToListAppend(), false));
