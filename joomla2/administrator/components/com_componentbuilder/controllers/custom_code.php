...
class ComponentbuilderControllerCustom_code extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('custom_code.edit', 'com_componentbuilder.custom_code.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('custom_code.edit.own', 'com_componentbuilder.custom_code.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('custom_code.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
		...
		return $user->authorise('custom_code.edit', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Custom_code', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=custom_codes' . $this->getRedirectToListAppend(), false));
