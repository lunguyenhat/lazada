...
class ComponentbuilderControllerComponent_config extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('component_config.edit', 'com_componentbuilder.component_config.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('component_config.edit.own', 'com_componentbuilder.component_config.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('component_config.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
		...
		return $user->authorise('component_config.edit', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Component_config', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=components_config' . $this->getRedirectToListAppend(), false));
