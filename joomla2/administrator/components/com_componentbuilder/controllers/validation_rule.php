...
class ComponentbuilderControllerValidation_rule extends JControllerForm
{
	...
	{
		$this->view_list = 'Validation_rules'; // safeguard for setting the return view listing to the main view.
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			if (!$permission)
			{
				if ($user->authorise('validation_rule.edit.own', 'com_componentbuilder.validation_rule.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('validation_rule.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
		...
		return $user->authorise('validation_rule.edit', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Validation_rule', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=validation_rules' . $this->getRedirectToListAppend(), false));
