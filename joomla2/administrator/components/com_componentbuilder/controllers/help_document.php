 ...
 */
class ComponentbuilderControllerHelp_document extends JControllerForm
{
	/**
		...
		if ($recordId)
		{
			...
			if (!$permission)
			{
				if ($user->authorise('help_document.edit.own', 'com_componentbuilder.help_document.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('help_document.edit.own', 'com_componentbuilder'))
						{
							return true;
						}
					}
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_componentbuilder&view=help_documents' . $this->getRedirectToListAppend(), false));
