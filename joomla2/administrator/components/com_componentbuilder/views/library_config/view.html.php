...
class ComponentbuilderViewLibrary_config extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_COMPONENTBUILDER_LIBRARY_CONFIG_NEW' : 'COM_COMPONENTBUILDER_LIBRARY_CONFIG_EDIT'), 'pencil-2 article-add');
		...
		if ($this->refid || $this->ref)
		{
			if ($this->canDo->get('library_config.create') && $isNew)
			{
				...
				JToolBarHelper::save('library_config.save', 'JTOOLBAR_SAVE');
			}
			...
			if ($isNew)
			{
				...
				JToolBarHelper::cancel('library_config.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				JToolBarHelper::cancel('library_config.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				{
					JToolBarHelper::apply('library_config.apply', 'JTOOLBAR_APPLY');
					JToolBarHelper::save('library_config.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('library_config.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('library_config.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('library_config.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('library_config.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_componentbuilder.library_config', $this->item->id);
				}
				if ($this->canDo->get('library_config.create'))
				{
					JToolBarHelper::custom('library_config.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('library_config.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = ComponentbuilderHelper::getHelpUrl('library_config');
