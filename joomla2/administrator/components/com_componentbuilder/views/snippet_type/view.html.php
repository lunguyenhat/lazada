...
class ComponentbuilderViewSnippet_type extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_COMPONENTBUILDER_SNIPPET_TYPE_NEW' : 'COM_COMPONENTBUILDER_SNIPPET_TYPE_EDIT'), 'pencil-2 article-add');
		...
		if ($this->refid || $this->ref)
		{
			if ($this->canDo->get('snippet_type.create') && $isNew)
			{
				...
				JToolBarHelper::save('snippet_type.save', 'JTOOLBAR_SAVE');
			}
			...
			if ($isNew)
			{
				...
				JToolBarHelper::cancel('snippet_type.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				JToolBarHelper::cancel('snippet_type.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				{
					JToolBarHelper::apply('snippet_type.apply', 'JTOOLBAR_APPLY');
					JToolBarHelper::save('snippet_type.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('snippet_type.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('snippet_type.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('snippet_type.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('snippet_type.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_componentbuilder.snippet_type', $this->item->id);
				}
				if ($this->canDo->get('snippet_type.create'))
				{
					JToolBarHelper::custom('snippet_type.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('snippet_type.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = ComponentbuilderHelper::getHelpUrl('snippet_type');
