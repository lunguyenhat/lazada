...
class ComponentbuilderViewLibrary extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_COMPONENTBUILDER_LIBRARY_NEW' : 'COM_COMPONENTBUILDER_LIBRARY_EDIT'), 'pencil-2 article-add');
		...
		if ($this->refid || $this->ref)
		{
			if ($this->canDo->get('library.create') && $isNew)
			{
				...
				JToolBarHelper::save('library.save', 'JTOOLBAR_SAVE');
			}
			...
			{
				// We can close it.
				JToolBarHelper::cancel('library.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('library.create'))
				{
					...
					JToolBarHelper::save('library.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('library.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('library.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('library.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('library.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_componentbuilder.library', $this->item->id);
				}
				if ($this->canDo->get('library.create'))
				{
					JToolBarHelper::custom('library.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('library.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = ComponentbuilderHelper::getHelpUrl('library');
