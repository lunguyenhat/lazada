...
class ComponentbuilderViewLayout extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_COMPONENTBUILDER_LAYOUT_NEW' : 'COM_COMPONENTBUILDER_LAYOUT_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('core.create'))
				{
					...
					JToolBarHelper::save('layout.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('layout.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('layout.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('layout.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('core.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_componentbuilder.layout', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
					JToolBarHelper::custom('layout.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('layout.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = ComponentbuilderHelper::getHelpUrl('layout');
