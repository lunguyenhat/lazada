...
class DemoModelImport extends JModelLegacy
{
	 ...
	 * @var        string
	 */
	protected $_context = 'com_demo.import';
	...
	public function import()
	{
		...
		if ($package === null)
		{
			switch ($this->getType)
			{
				...
				default:
					$app->setUserState('com_demo.message', JText::_('COM_DEMO_IMPORT_NO_IMPORT_TYPE_FOUND'));
		...
		if (!$package || !$package['type'])
		{
			...
			$app->setUserState('com_demo.message', JText::_('COM_DEMO_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
			return false;
		}
			...
			if(DemoHelper::checkArray($data['array']) && $user->authorise($table.'.import', 'com_demo') && $user->authorise('core.import', 'com_demo'))
			{
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__demo_'.$table))
					...
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__demo_'.$table))
